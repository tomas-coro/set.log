...
function runMigration(db){
  ...
  vm.runInNewContext(`${extractFunction("migrateWorkoutV6")}; result=migrateWorkoutV6();`, context);
...
test("V6 sostituisce solo la scheda e conserva tutti i dati storici",()=>{
  ...
  const {result,saves}=runMigration(db);
  ...
  assert.deepEqual(withoutIds(db.schema.days),expectedDays);
